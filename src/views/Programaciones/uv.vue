      ...
      <v-col cols="12">
        ...
        <v-card outlined class="pa-3">
          <v-row class="pa-1 py-0">
						<v-col cols="12"  md="6" >
							<v-card-actions class="font-weight-black headline  py-0 mt-1 " > UV </v-card-actions>
						</v-col>
...
<script>
  ...
  export default {
    ...
    created(){
      ...
      if(this.Parametros_UV.length){
				this.estatus  = { id: this.Parametros_UV.estatus };
				this.fecha1   = this.Parametros_UV.fecha1;
				this.fecha2   = this.Parametros_UV.fecha2;
			}
      this.init();
    },
    ...
    computed:{
			...mapGetters('PrograDepto' ,['Loading','Parametros_UV','UV']), // IMPORTANDO USO DE VUEX - (GETTERS)
    ...
    methods:{ 
      ...mapActions('PrograDepto' ,['obtener_Programaciones_Uv','finalizar_Producto_Uv']), 
      ...
      init(){
        ...
        this.obtener_Programaciones_Uv(payload)
      },
      ...
      finalizarOT(){
        ...
        this.finalizar_Producto_Uv(payload).then(response =>{
      ...
      guardarMaquina(item){
        this.$http.put('actualiza.maquina.uv/'+ item.id, item);
        this.init();
      }
    }
