      ...
      <v-col cols="12">
        ...
        <v-card outlined class="pa-3">
          <v-row class="pa-1 py-0">
						<v-col cols="12"  md="6" >
							<v-card-actions class="font-weight-black headline  py-0 mt-1 " > SUBLIMACIÓN </v-card-actions>
						</v-col>
...
<script>
  ...
  export default {
    ...
    created(){
      ...
      this.consultar_Maqinas_Depto(2);
      this.sucursal = { id: this.getdatosUsuario.id_sucursal }

      if(this.Parametros_Sublimacion.length){
				this.estatus  = { id: this.Parametros_Sublimacion.estatus };
				this.fecha1   = this.Parametros_Sublimacion.fecha1;
				this.fecha2   = this.Parametros_Sublimacion.fecha2;
			}
      this.init();
    },
    ...
    computed:{
			...mapGetters('PrograDepto' ,['Loading','Parametros_Sublimacion','Sublimacion']), // IMPORTANDO USO DE VUEX - (GETTERS)
    ...
    methods:{ 
      ...mapActions('PrograDepto' ,['obtener_Programaciones_Sublimacion','finalizar_Producto_Sublimacion']), 
      ...
      init(){
        ...
        this.obtener_Programaciones_Sublimacion(payload)
      },
      ...
      finalizarOT(){
        ...
        this.finalizar_Producto_Sublimacion(payload).then(response =>{
      ...
      guardarMaquina(item){
        this.$http.put('actualiza.maquina.sublimacion/'+ item.id, item);
        this.init();
      }
    }
